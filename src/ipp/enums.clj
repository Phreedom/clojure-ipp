...
(defn ^:private reserved-range-map [first last]
  (into {} (map (fn [v] [v (keyword (str "reserved-" v))])
                (range first last))))
...
(def reserved-type-to-tag
  (map-invert tag-to-reserved-type))
...
(def attribute-value-type-to-tag
  (map-invert tag-to-attribute-value-type))
...
(def supported-attribute-groups
  (merge {
    ...
    ; these values are defined in the spec as attribute groups,
    ; and we must skip them correctly
    (reserved-range-map 0x06 0x0f)))

(def attribute-group-type-to-code
  (map-invert supported-attribute-groups))
...
(def operation-to-tag
  (map-invert tag-to-operation))
